...
@patch('zefoy_bot.bot.sleep', return_value=None)
...
def test_init_csv_creates_file():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        import zefoy_bot.bot as botmod
        old_csv = botmod.CSV_FILE
        botmod.CSV_FILE = test_csv
        try:
            init_csv()
...
@patch('zefoy_bot.bot.sleep', return_value=None)
@patch('zefoy_bot.bot.decode')
@patch('zefoy_bot.bot.http_request')
...
def test_search_link_timer_retry(mock_http, mock_decode, mock_sleep):
    ...
    session = MagicMock()
    with patch('zefoy_bot.bot.wait_timer'):
...
def test_search_link_fcde_form(mock_http, mock_decode, mock_sleep):
    ...
    session = MagicMock()
    with patch('zefoy_bot.bot.send_action', return_value=True) as mock_send:
...
def test_save_and_load_config(tmp_path):
    ...
    with patch('zefoy_bot.bot.CONFIG_FILE', config_path):
        loaded = load_config()
